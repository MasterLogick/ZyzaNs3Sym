...
ZyzaInfiniteClient::ZyzaInfiniteClient(
    int clientId, int nodesCount,
    std::vector<std::vector<uint8_t>> &serializedPublicKeys,
    ns3::PointToPointStarHelper &p2psh,
    std::chrono::milliseconds requestTimeout)
    : clientId(clientId), nodesCount(nodesCount),
      serializedPublicKeys(serializedPublicKeys), p2psh(p2psh),
      requestTimeout(requestTimeout), currentClient() {}

void ZyzaInfiniteClient::StartApplication() { startNextRound(); }

void ZyzaInfiniteClient::startNextRound() {
  roundStart = ns3::Simulator::Now();
  uint16_t leaderHint = 0xffff;
  if (currentClient != nullptr) {
    leaderHint = currentClient->getLeaderHint();
  }
  currentClient = std::make_unique<zyza::ZyzaClientRequest>(
      leaderHint, clientId, nodesCount, serializedPublicKeys, p2psh,
      requestTimeout);
  zyza::ZyzaCommon::fillRandom(message, sizeof(message));
