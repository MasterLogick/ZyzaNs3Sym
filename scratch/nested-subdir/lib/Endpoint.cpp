...
Endpoint::Endpoint(ns3::Ptr<ns3::Node> thisNode)
    : thisNode(thisNode), inDestructor(false) {
  lifetimePointer = std::make_shared<int>();
}

void Endpoint::run() {
  serverTcpSocket =
      ns3::Socket::CreateSocket(thisNode, ns3::TcpSocketFactory::GetTypeId());
  ...
  serverTcpSocket->SetAcceptCallback(
      ...
      [this, weakPointer = std::weak_ptr<int>(lifetimePointer)](
          ns3::Ptr<ns3::Socket> a, auto &b) -> void {
        ...
        a->SetRecvCallback([this, pim,
                            weakPointer = std::weak_ptr<int>(lifetimePointer)](
          ...
          while (auto packet = a->Recv()) {
            ...
            if (pim->totalSize != 0) {
              auto s =
                  std::min<size_t>(pim->totalSize - pim->read, remainderSize);
            ...
            while (remainderSize >= 4) {
              ...
              if (remainderSize >= nextMsgSize) {
                onTcpMessage(
                    {reinterpret_cast<uint8_t *>(remainderPtr), nextMsgSize});
  ...
  serverUdpSocket =
      ns3::Socket::CreateSocket(thisNode, ns3::UdpSocketFactory::GetTypeId());
  if (serverUdpSocket->Bind(
          ns3::InetSocketAddress(ns3::Ipv4Address::GetAny(), 1235)) == -1) {
    NS_FATAL_ERROR("Failed to bind socket");
  }
  serverUdpSocket->SetRecvCallback(
      [this, weakPointer =
                 std::weak_ptr<int>(lifetimePointer)](ns3::Ptr<ns3::Socket> s) {
        auto p = weakPointer.lock();
        if (p == nullptr) {
          return;
        }
